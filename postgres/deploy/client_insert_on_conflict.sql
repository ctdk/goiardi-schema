...
CREATE OR REPLACE FUNCTION goiardi.merge_clients(m_name text, m_nodename text, m_validator boolean, m_admin boolean, m_public_key text, m_certificate text, m_organization_id bigint) RETURNS VOID AS
$$
DECLARE
    u_id bigint;
    u_name text;
BEGIN
    ...
    INSERT INTO goiardi.clients (name, nodename, validator, admin, public_key, certificate, created_at, updated_at, organization_id)
        ...
        ON CONFLICT(organization_id, name)
            DO UPDATE SET nodename = m_nodename, validator = m_validator, admin = m_admin, public_key = m_public_key, certificate = m_certificate, updated_at = NOW();
END;
$$
LANGUAGE plpgsql;
