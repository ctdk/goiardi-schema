...
BEGIN
	...
	UPDATE goiardi.organizations SET full_name = m_full_name, guid = m_guid, uuid = m_uuid, updated_at = NOW() WHERE name = m_name;
	IF found THEN
	    RETURN;
	END IF;
        ...
        BEGIN
	    INSERT INTO goiardi.organizations (name, description, guid, uuid, created_at, updated_at) VALUES (m_name, m_description, m_guid, m_uuid, NOW(), NOW());
            RETURN;
